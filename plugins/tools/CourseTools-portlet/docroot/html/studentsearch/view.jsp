...
<aui:form name="studentsearch" action="<%=buscarURL %>" method="post">
	<aui:fieldset>
		<aui:column>
			<aui:input label="studentsearch.criteria" name="criteria" size="20" value="<%=criteria %>" ignoreRequestValue="<%=true %>"  />	
		</aui:column>	
		<aui:button-row>
...
<liferay-ui:search-container iteratorURL="<%=portletURL%>" emptyResultsMessage="there-are-no-results" delta="10" deltaConfigurable="true">
   	...
   	<liferay-ui:search-container-results>
	    ...
	    for(User u:userListPage){ids+=u.getUserId()+";";}
		%>
	</liferay-ui:search-container-results>
	...
	<liferay-ui:search-container-row className="com.liferay.portal.model.User" keyProperty="userId" modelVar="user">
		...
		<liferay-ui:search-container-column-text name="Id">
			<div class="userId"><%=user.getUserId() %></div>
		</liferay-ui:search-container-column-text>
