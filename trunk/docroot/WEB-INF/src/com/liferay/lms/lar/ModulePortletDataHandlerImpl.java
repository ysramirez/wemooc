...
public class ModulePortletDataHandlerImpl implements PortletDataHandler {
	...
	private static final String _EXPORT_SAMPLE_LAR_DATA = "export-sample-lar-portlet-data";
	private static final String _TYPE_CSV = "csv";
	private static final String _TYPE_XML = "xml";
	
	private static final PortletDataHandlerBoolean _createReadme = new PortletDataHandlerBoolean (_NAMESPACE, _CREATE_README, true);
	private static final PortletDataHandlerChoice _dataType = new PortletDataHandlerChoice(_NAMESPACE,_DATA_TYPE, 1, new String[] { _TYPE_CSV, _TYPE_XML });
	private static final PortletDataHandlerBoolean _enableExport = new PortletDataHandlerBoolean(_NAMESPACE, _EXPORT_SAMPLE_LAR_DATA, true, 
	...
	public PortletDataHandlerControl[] getExportControls() throws PortletDataException {
		System.out.println(" getExportControls : "   );
	...
	public PortletDataHandlerControl[] getExportMetadataControls() throws PortletDataException {
		System.out.println(" getExportMetadataControls : "   );
		return new PortletDataHandlerControl[] {_enableExport};
	}
	...
	@Override
	public PortletPreferences deleteData(PortletDataContext arg0, String arg1,
			PortletPreferences arg2) throws PortletDataException {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public String exportData(PortletDataContext context, String portletId, PortletPreferences arg2) throws PortletDataException {
		
		System.out.println(" exportData : " + portletId  );
	...
	private void exportEntry(PortletDataContext context, Element root, Module entry) throws PortalException, SystemException {
		...
		for(LearningActivity actividad:actividades)
		{
			...
			if(actividad.getTypeId() == 2){
				...
				String document = LearningActivityLocalServiceUtil.getExtraContentValue(actividad.getActId(), "document");
			
				//Si tenemos un documento en el resource.
				if(!document.equals("")){
				
					try {
						AssetEntry docAsset= AssetEntryLocalServiceUtil.getAssetEntry(Long.valueOf(document));
		...
		for (Element actElement : entryElement.elements("learningactivity")) {
			...
			if(actElement.element("dlfileentry") != null){
				...
				try {
					...
					//DLFileEntry newFile = DLFileEntryLocalServiceUtil.addFileEntry(serviceContext.getUserId(),larn.getGroupId(), larn.getGroupId(), false, dlFolder.getFolderId(), oldFile.getName(), oldFile.getTitle(), oldFile.getDescription(), "", "", IOUtils.toByteArray(is), serviceContext);
					DLFileEntry newFile = DLFileEntryLocalServiceUtil.addFileEntry(serviceContext.getUserId(), larn.getGroupId(), larn.getGroupId(), dlFolder.getFolderId(), oldFile.getName(), oldFile.getMimeType(), oldFile.getTitle(), oldFile.getDescription(), "Importation", oldFile.getFileEntryTypeId(), null, null , is, oldFile.getSize(), serviceContext);
					

					
					/*
					DLFileEntry newFile = DLFileEntryLocalServiceUtil.createDLFileEntry(CounterLocalServiceUtil.increment(DLFileEntry.class.getName()));
					newFile.setUserId(arg0);
					newFile.setGroupId(arg0);
					newFile.setRepositoryId(arg0);
					newFile.setFolderId(arg0);
					newFile.setName(arg0);
					newFile.setMimeType(arg0);
					newFile.setTitle(arg0)
					newFile.setDescription(arg0)
					newFile.
					newFile.set
					newFile.set
					
					DLFileEntryLocalServiceUtil.addDLFileEntry(newFile);
					*/
	...
	/*
	 * Busca el directorio padre, si no existe lo crea.
	 */
	private DLFolder getMainDLFolder(Long userId, Long groupId, ServiceContext serviceContext) throws PortalException, SystemException{
		...
		try {
        	...
        	mainFolder = DLFolderLocalServiceUtil.addFolder(userId, groupId, groupId, false, 0, "ResourceUploads", "ResourceUploads", serviceContext);
	...
	private DLFolder createDLFoldersForLearningActivity(Long userId, Long groupId, Long actId, String title, ServiceContext serviceContext) throws PortalException, SystemException{
		...
		try {
			...
			if(newDLFolder == null){
				...
				newDLFolder = DLFolderLocalServiceUtil.addFolder(userId, groupId, (long)0, false, dlMainFolder.getFolderId(),  String.valueOf(actId), title, serviceContext);
