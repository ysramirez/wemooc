...
function __loadCompatLayer(w) {
	...
	var attachEventProxy = function(eventName, eventHandler) {
		...
		this.addEventListener(eventName.slice(2),
				eventHandler._mozillaEventHandler, false);
	};
	var detachEventProxy = function(eventName, eventHandler) {
		if (eventHandler._mozillaEventHandler) {
			...
			this.removeEventListener(eventName.slice(2), mozillaEventHandler,
					false);
		}
	};
	...
	w.Event.prototype.__defineGetter__('offsetX', function() {
		return window.pageXOffset + this.clientX
				- __getLocation(this.srcElement).x;
	});
	w.Event.prototype.__defineGetter__('offsetY', function() {
		return window.pageYOffset + this.clientY
				- __getLocation(this.srcElement).y;
	});
	...
	var selectNodes = function(doc, path, contextNode) {
		...
		var result = xpath.evaluate(path, contextNode, doc
				.createNSResolver(doc.documentElement),
	...
	w.XMLDocument.prototype.transformNode = function(xsl) {
		...
		var ownerDocument = document.implementation
				.createDocument("", "", null);
...
function __supportsCompatLayer(ua) {
	return (ua.indexOf('Gecko') >= 0) || (ua.indexOf('AppleWebKit') >= 0)
			|| (ua.indexOf('Opera') >= 0);
}
