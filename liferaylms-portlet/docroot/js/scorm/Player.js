...
Player.PersistentStateStorage.prototype = {
	...
	saveState : function(adlData, stasuses, globalObjectives,
			resumeAllIdentifier, suspendedGlobalObjectives) {
		var $0 = this.$0.getObjectItem('scormpool');
		var $1 = $0['organizations'];
		var $2 = $1[this.$1];
...
Player.ContentPlayer = function(manifestPath, imsmanifest, isStorage,
		isScormAuto, isDebug) {
	this.$2 = 0;
	this.$18 = [];
	...
	if (!isNullOrUndefined($0) && $0.length === 1) {
		if (API_BASE.BaseUtils.getText($0[0]) === '1.2' || API_BASE.BaseUtils.getText($0[0]).startsWith('1.1')) {
			this.$2 = 1;
				...
				'1.3') >= 0
				|| API_BASE.BaseUtils.getText($0[0]).toLowerCase().indexOf(
						'2004') >= 0) {
			this.$2 = 2;
		}
	} else {
		if (($1.attributes.getNamedItem('xmlns') != null && API_BASE.BaseUtils
				.getText($1.attributes.getNamedItem('xmlns')) === 'http://www.imsproject.org/xsd/imscp_rootv1p1p2')
				|| ($1.attributes.getNamedItem('version') != null && API_BASE.BaseUtils
						.getText($1.attributes.getNamedItem('version')) === '1.1')) {
			this.$2 = 1;
		}
	}
	if (this.$2 === 0) {
	...
	if (isStorage) {
		if ($3 == null) {
			var $4 = API_BASE.BaseUtils.getChildSiblingsByName($2,
					'organization');
	...
	if (this.$2 === 1) {
		this.$9 = new SCORM_1_2.ActivityTree($3, ((this.$17 != null) ? this.$17
				.getAllStatus() : null), isScormAuto);
	} else {
		this.$9 = new SCORM_1_3.ActivityTree(
				$3,
				(this.$17 != null) ? this.$17.getResumeAllIdentifier() : null,
				((this.$17 != null) ? this.$17.getAllStatus() : null),
				((this.$17 != null) ? (this.$17.getResumeAllIdentifier() != null) ? this.$17
						.getSuspendedGlobalObjectives()
						: this.$17.getAllObjectivesGlobaltoSystem()
						: null));
		if (this.$17 != null) {
...
Player.ContentPlayer.prototype = {
	...
	$20 : function($p0) {
		...
		$2.innerHTML = $1.getHours() + ':' + $1.getMinutes() + ':'
				+ $1.getSeconds();
		...
		if (!isNullOrUndefined($p0.errorCode) && $p0.errorCode !== '0') {
			...
			$4.innerHTML = '<b>Error Code:</b> '
					+ $p0.errorCode
					+ ((isNullOrUndefined($p0.errorDescription) || $p0.errorDescription.length === 0) ? ''
							: '<br/><b>Error Info:</b> ' + $p0.errorDescription);
			$3.appendChild($4);
		}
		$0.appendChild($2);
		$0.appendChild($3);
	...
	$21 : function($p0, $p1) {
		...
		if (isNullOrUndefined($p1.get_node())
				|| isNullOrUndefined($p1.get_node().getAnchor().getAttribute(
						'href'))
				|| $p1.get_node().getAnchor().getAttribute('href') === '') {
			return;
		}
		var $0 = $p1.get_node().get_userData();
		if (isNullOrUndefined($0) || $0.getIdentifier() == null
				|| $0.getIdentifier().length <= 0) {
			return;
		}
		...
		if (this.$2 === 2) {
			if ($0) {
				...
				if (!isNullOrUndefined(this.$10) && !$1.contains('previous')) {
					this.$10.disabled = !$3.getParentSequencing().flow
							|| $3.getParentSequencing().forwardOnly
							|| !$2.isValidNavigationRequest('previous', false);
				}
				if (!isNullOrUndefined(this.$11) && !$1.contains('continue')) {
					this.$11.disabled = !$3.getParentSequencing().flow
							|| !$2.isValidNavigationRequest('continue', false);
				}
			}
			this.$19 = 0;
	...
	$24 : function() {
		if (this.$19 < this.$18.length) {
			...
			var $3 = this.$C;
			if ($3 !== $2
					&& $0.isValidNavigationRequest('choice.{target='
							+ $2.getIdentifier() + '}', false)) {
				$1.getAnchor().href = '#';
			} else {
	...
	$25 : function() {
		if (this.$19 < this.$18.length) {
			...
			var $3 = this.$C;
			if ($3 !== $2
					&& $0.isValidNavigationRequest('{target='
							+ $2.getIdentifier() + '}choice')) {
				$1.getAnchor().href = '#';
			} else {
				$1.getAnchor().removeAttribute('href');
			}
	...
	$28 : function($p0) {
		if ($p0 != null) {
			...
			if (!isNullOrUndefined(this.$12)) {
				this.$12.style.display = ($1.contains('exit')) ? 'none'
						: 'inline';
			...
			if (!isNullOrUndefined(this.$14)) {
				this.$14.style.display = ($1.contains('abandon')) ? 'none'
						: 'inline';
				this.$14.disabled = false;
			}
			if (!isNullOrUndefined(this.$15)) {
				this.$15.style.display = ($1.contains('abandonall')) ? 'none'
						: 'inline';
				this.$15.disabled = false;
			}
			if (!isNullOrUndefined(this.$16)) {
				this.$16.style.display = ($1.contains('suspendall')) ? 'none'
						: 'inline';
...
Run.ManifestByString = function(manifest) {
	...
	if (isNullOrUndefined(Run.$1)
			|| isNullOrUndefined(API_BASE.BaseUtils.getChildNodeByName(Run.$1,
					'manifest'))) {
		API_BASE.LOG.displayMessage('[PLR.1] Loading course process',
				'PLR.1.1', 'Incorrect imsmanifest.xml');
		return;
	}
	Run.$4();
}
...
Run.$4 = function() {
	var $0 = new Player.ContentPlayer(Run.$2, Run.$1,
